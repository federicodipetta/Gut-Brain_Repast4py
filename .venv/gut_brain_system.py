...
class Sleep(core.Agent):
    TYPE = 9

    def __init__(self, local_id: int, rank: int, pt: dpt, context):
        super().__init__(id = local_id, type = Sleep.TYPE, rank = rank)
        self.pt = pt
        self.context = context
        self.hours_of_sleep = params['hours_of_sleep.count']
        self.input_name = "sleep"
    ...
    def step(self):
        def add_oligomers():
            #  Calculate the number of oligome to add considering the number of hours of sleep
            to_add = int(((abs(self.hours_of_sleep - 8 )) * np.random.uniform(0, 3)))
            for _ in range(to_add):
                model.add_oligomer_protein('tau', 'brain')
        if self.hours_of_sleep >= 8:
            return
        # add_oligomers()
...
class Model():
    ...
    def __init__(self, comm: MPI.Intracomm, params: Dict):
        ...
        agent_types_brain = [
            ...
            ('hours_of_sleep.count', Sleep, None)
        ]
